...
$allowed = ['spotify_followers','spotify_listeners','youtube_subs','youtube_video',
            ...
            'instagram_photos'];
// instagram_avatar è a parte: non solo si salva in stats, serve anche per ricalcolare photo_url
// (l'URL CDN Instagram scade dopo pochi giorni, va rinfrescato — vedi worker/social-stats.php).
$allowedForPhoto = 'instagram_avatar';

$sel = db()->prepare('SELECT stats, socials, photo_url FROM artist_profiles WHERE user_id = ?');
$upd = db()->prepare('UPDATE artist_profiles SET stats = ?, stats_updated_at = NOW() WHERE user_id = ?');
$updPhoto = db()->prepare('UPDATE artist_profiles SET stats = ?, stats_updated_at = NOW(), photo_url = ? WHERE user_id = ?');
$n = 0;
foreach ($items as $it) {
  ...
  $row = $sel->fetch();
  if (!$row) continue;
  $stats = json_decode($row['stats'] ?? '', true) ?: [];   // parti dalle esistenti
  foreach (($it['stats'] ?? []) as $k => $v) {
    if (in_array($k, $allowed, true) && $v !== null && $v !== '') $stats[$k] = is_numeric($v) ? (int) $v : $v;
  }
  $newAvatar = trim((string) ($it['stats'][$allowedForPhoto] ?? ''));
  if ($newAvatar !== '') {
    $stats[$allowedForPhoto] = $newAvatar;
    $socials = json_decode($row['socials'] ?? '', true) ?: [];
    $firstGenreSlug = resolve_first_genre_slug($uid, null);
    [$photo, ] = resolve_photo_url($row['photo_url'] ?? null, $socials, $firstGenreSlug, $newAvatar, $uid);
    $updPhoto->execute([$stats ? json_encode($stats, JSON_UNESCAPED_UNICODE) : null, $photo, $uid]);
  } else {
